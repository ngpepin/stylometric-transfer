...
# Wrapper script for fingerprint_api_harness.py.
...
# Default API port for the GUI harness.
API_PORT=8765
...
# Delegate to the Python entry point. --api can still be overridden by later args.
exec python3 "${REPO_ROOT}/fingerprint_api_harness.py" --api "${API_PORT}" "$@"
